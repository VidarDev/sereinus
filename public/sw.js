...
const CACHE_CONFIG = {
	images: { type: CACHE_TYPES.CACHE_FIRST, maxAge: 86400000 }, // 24h
	static: { type: CACHE_TYPES.CACHE_FIRST, maxAge: 604800000 }, // 7d
	api: { type: CACHE_TYPES.NETWORK_FIRST, maxAge: 300000 }, // 5m
	pages: { type: CACHE_TYPES.STALE_WHILE_REVALIDATE, maxAge: 86400000 } // 24h
};
...
self.addEventListener("fetch", (event) => {
	...
	switch (cacheStrategy.type) {
		case CACHE_TYPES.CACHE_FIRST:
			event.respondWith(handleCacheFirst(request, cacheStrategy));
			break;

		case CACHE_TYPES.NETWORK_FIRST:
...
const getCacheStrategy = (request) => {
	...
	if (isRequestForImage(request, pathname)) {
		return CACHE_CONFIG.images;
	}

	if (isRequestForStaticAssets(request, pathname)) {
		return CACHE_CONFIG.static;
	}

	if (isRequestForApi(pathname)) {
		return CACHE_CONFIG.api;
	}

	if (isRequestForPages(request)) {
...
const isRequestForImage = (request, pathname) => {
	return request.destination === "image" ||
		/\.(jpg|jpeg|png|gif|webp|svg|ico)$/i.test(pathname);
};

const isRequestForStaticAssets = (request, pathname) => {
...
const isRequestForApi = (pathname) => {
	return pathname.startsWith("/api/") ||
		pathname.includes("/actions/");
};

const isRequestForPages = (request) => {
	return request.mode === "navigate" ||
		request.destination === "document";
};

const handleCacheFirst = async (request, config) => {
	try {
...
const handleNetworkFirst = async (request, config) => {
	try {
		const networkResponse = await fetch(request);
...
const handleStaleWhileRevalidate = async (request, config) => {
	const cachedResponse = await caches.match(request);
...
const isExtensionMessage = (event) => {
